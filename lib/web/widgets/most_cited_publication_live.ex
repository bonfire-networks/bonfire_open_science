defmodule Bonfire.OpenScience.MostCitedPublicationLive do
  ...
  def update(assigns, socket) do
    ...
    case Publications.get_most_cited_publication(user) do
      {:ok, publication} ->
        {:ok, assign(socket, most_cited_publication: publication)}

      {:error, reason} ->
        debug(reason, "Could not fetch most cited publication")
        {:ok, assign(socket, most_cited_publication: nil)}
    end
  end
  ...
  def get_citation_class(count) when is_integer(count) do
    cond do
      count >= 100 -> "badge-warning"
      count >= 50 -> "badge-info"
      count >= 10 -> "badge-accent"
      true -> "badge-neutral"
    end
  end

  def get_citation_class(_), do: "badge-neutral"
